...
- 公開 URL での危険設定チェック
  ...
  - `BOOKING_DATA_ENCRYPTION_KEY` 未設定または Fernet 形式でなければ起動停止
  - `BOOKING_SEED_DEMO=true` なら起動停止
  - `ACTIONS_DRY_RUN=true` なら起動停止
- `/api/booking/keepalive` は `BOOKING_CRON_SECRET` 設定時のみ Bearer / `X-Cron-Secret` を要求
- `/api/booking/jobs/run` は `BOOKING_CRON_SECRET` / `CRON_SECRET` または `X-Admin-Secret` で認証し、ジョブ重複実行を防止
...
- `BOOKING_DATA_ENCRYPTION_KEY=Fernet形式の32バイトbase64キー`
...
- `BOOKING_CRON_SECRET=長いランダム文字列`
- `BOOKING_GOOGLE_DELETE_CHECK_INTERVAL_SEC=60`
...
- Set `CRON_SECRET` or `BOOKING_CRON_SECRET` to the same long random value. Vercel Cron sends it as `Authorization: Bearer ...`.
...
Vercel Hobby Cron is suitable only as a daily safety net. As of January 28, 2026, Hobby cron schedules are limited to once per day and hourly precision. Use an external cron service, GitHub Actions schedule, or a paid Vercel plan if reminders and Google sync need to run more frequently than daily. The same endpoint can be called externally:

```bash
curl -X POST https://your-app.example.com/api/booking/jobs/run \
